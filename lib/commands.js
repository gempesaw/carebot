...
import kill from '~/lib/commands/kill';
...
const commands = {
    ...
    kill: {
        action: kill.kill,
        description: kill.kill.description
    },
    lunch: {
        action: () => lunchDeals(),
    ...
    unset: {
        action: getSet.unset,
        description: getSet.unset.description
    },
...
async function outputHelp(prefix = '', commands = getFlowbotCommands()) {
    ...
    const commandsWithHelp = commandNames
          .filter(it => ! /^(?:cancel|deploy|contexts|kill|help)$/.test(it))
          .map(cmd => {
              const name = /-+/.test(cmd) ? cmd : `- \`.${cmd}\``;
              const desc = R.prop('description', commands[cmd] || {});
              if (desc) {
                  return `${name}: ${desc}`;
              }
              else {
