...
queue.description = 'Do a build update & service restart on KMS only; accepts one argument: the name of the context.';
...
function parseContext(event) {
    ...
    const expandedContext = expandContext(context);
    return expandedContext || context;
}
...
function expandContext(context) {
    if (! /(\w-)+\w/.test(context)) {
        ...
        const contexts = bcsd.getContexts();
        const expansionRegex = new RegExp(`^${context.split('-').join('.*-')}\\w+$`);
        return contexts.find(expansionRegex.test.bind(expansionRegex));
    }
}

function contexts(event) {
    const contexts = bcsd.getContexts();
    const shorthands = contexts.reduce((acc, it) => {
        const short = it
              .split('-')
